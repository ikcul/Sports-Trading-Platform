...
def test_recommendation_rejects_low_edge() -> None:
    model = EnsembleOutput(model_name="weighted_ensemble", match_id="m1", home_win=0.46, draw=0.28, away_win=0.26, expected_home_goals=1.4, expected_away_goals=1.1, confidence_interval=(0.39, 0.53), variance=2.5, calibration_error=0.04, component_weights={"elo": 0.5, "poisson_goals": 0.5}, model_agreement=0.91)
...
def test_recommendation_allows_high_quality_positive_ev() -> None:
    model = EnsembleOutput(model_name="weighted_ensemble", match_id="m1", home_win=0.55, draw=0.25, away_win=0.20, expected_home_goals=1.8, expected_away_goals=0.9, confidence_interval=(0.49, 0.61), variance=2.0, calibration_error=0.03, component_weights={"elo": 0.35, "poisson_goals": 0.45, "bayesian_context": 0.20}, model_agreement=0.88)
